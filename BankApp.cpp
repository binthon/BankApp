    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (textField1.getGlobalBounds().contains(window.mapPixelToCoords(sf::Mouse::getPosition(window)))) {
                ...
                else if (submitButton.getGlobalBounds().contains(window.mapPixelToCoords(sf::Mouse::getPosition(window)))) {
                    cout << "Submit";
                }
                else {
                    isTextField1Active = false;
            ...
            if (event.type == sf::Event::TextEntered) {
                if (isTextField1Active) {
                    if (event.text.unicode == '\b' && !inputText1.empty()) { // Backspace
                        inputText1.pop_back();
                    }
                    else if (event.text.unicode < 128 && event.text.unicode != '\b') { // Normal character
                        inputText1 += static_cast<char>(event.text.unicode);
                    }
                }
                else if (isTextField2Active) {
                    if (event.text.unicode == '\b' && !inputText2.empty()) { // Backspace
                        inputText2.pop_back();
                    }
                    else if (event.text.unicode < 128 && event.text.unicode != '\b') { // Normal character
